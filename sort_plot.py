...
def student_list(data_dictionary: dict) -> list:
    ...
    dictionary_sort = None #Create a variable for the header that the dictionary is sorted by

    for key in data_dictionary: #Iterate through each key in the original dictionary
        for data_set in data_dictionary[key]: #Iterate through each dictionary in the list assigned to the key
            ...
            """else: #If none of the above headers are missing then assume that the wrong dictionary was used
                ...
                exit"""
    temporary_dictionary = {dictionary_sort: None} #Create a temporary dictionary that has the header
    for key_value in data_dictionary: #Iterate through each key in the original dictionary
        for data_headers in data_dictionary[key_value]: #Iterate through each dictionary in the list assigned to the key
            temporary_dictionary[dictionary_sort] = key_value #Overwrite the temporary dictionary to have the key type and value
            data_headers.update(temporary_dictionary) #Add they key type and value back into the dictionary as another header
            data_list.append(data_headers) #Add the dictionary to the end of the data list
...
def curve_fit(generated_dictionary: dict, attribute: str, degree_of_polynomial: int) -> list:
    ...
    for averages in average_total:
        attribute_averages[averages] = (average_total[averages] / average_divider[averages])
    index_count = 0
    ...
    if len(y_values) - 1 < degree_of_polynomial:
        return np.polyfit(x_values, y_values, len(y_values) - 1)
    else:
        return np.polyfit(x_values, y_values, degree_of_polynomial)
#Function 5--------------------------------------------------------------------------------------------------------------------------#
...
#Histogram Tests ---------------------------------------------------------------------------------------------------------------#
    ...
    for i in labels:
        ...
        print(x)
#Curve fitting tests
    categories = ['Health', 'Age', 'Failures', 'StudyTime', 'Absences', 'G1', 'G2', 'G3']
    for poly_iteration in range(1, 6):
        for category_test in categories:
            print(curve_fit(data_age, category_test, poly_iteration), 'age', category_test)
            print(curve_fit(data_school, category_test, poly_iteration), 'school', category_test)
            print(curve_fit(data_health, category_test, poly_iteration), 'health', category_test)
            print(curve_fit(data_failures, category_test, poly_iteration), 'failures', category_test)
